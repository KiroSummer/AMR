...
class DynamicDataLoader(object):
    ...
    def read_a_part_data(self, num_sentences=50000):
        ...
        for amr, token, lemma, pos, ner, edge, dep_rel in zip(*dynamically_read_file(self.__large_file, max_sentence_length=num_sentences)):
            if self.train:
                _, _, not_ok = amr.root_centered_sort()
                if not_ok or len(token) == 0:
                    continue
